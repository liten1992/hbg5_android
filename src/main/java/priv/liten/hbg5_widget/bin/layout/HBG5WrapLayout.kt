...
/** 自動換行布局
 * By HuangJiaHeng 2020/4/14.
 * https://github.com/pimaryschoolstudent/WrapLayout/blob/master/wraplayout/src/main/java/primary/student/wraplayout/WrapLayout.kt
 *
 * */
class HBG5WrapLayout: ViewGroup {
    // MARK:- ====================== Constructor
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int): super(context, attrs, defStyleAttr) {
        val res = context.obtainStyledAttributes(attrs, R.styleable.HBG5WrapLayout)
        gravity = res.getInt(R.styleable.HBG5WrapLayout_android_gravity, Gravity.START)
        res.recycle()
    }
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        val sizeWidth = measuredWidth - paddingLeft - paddingRight

        when (gravity.and(Gravity.HORIZONTAL_GRAVITY_MASK)){
            Gravity.START,
            Gravity.LEFT -> gravityStartLayout(sizeWidth)
            Gravity.END,
            Gravity.RIGHT -> gravityEndLayout(sizeWidth)
            Gravity.CENTER_HORIZONTAL -> gravityCenterLayout(sizeWidth)
            //else -> gravityEndLayout(sizeWidth)
        }
    }
    ...
    private fun getChildViewWidthSize(childView: View?):Int {
        val clp = childView?.layoutParams as MarginLayoutParams
